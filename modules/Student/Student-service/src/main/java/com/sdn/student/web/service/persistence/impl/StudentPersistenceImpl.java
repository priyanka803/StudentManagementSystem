...
package com.sdn.student.web.service.persistence.impl;
...
import com.sdn.student.web.model.StudentTable;
import com.sdn.student.web.model.impl.StudentImpl;
import com.sdn.student.web.model.impl.StudentModelImpl;
import com.sdn.student.web.service.persistence.StudentPersistence;
import com.sdn.student.web.service.persistence.StudentUtil;
import com.sdn.student.web.service.persistence.impl.constants.STUDENTPersistenceConstants;
...
public class StudentPersistenceImpl
	...
	private FinderPath _finderPathFetchByFirstName;
	private FinderPath _finderPathCountByFirstName;

	/**
	 * Returns the student where firstName = &#63; or throws a <code>NoSuchStudentException</code> if it could not be found.
	 *
	...
	public Student findByFirstName(String firstName)
		...
		Student student = fetchByFirstName(firstName);
	...
	/**
	 * Returns the student where firstName = &#63; or returns <code>null</code> if it could not be found. Uses the finder cache.
	 *
	 * @param firstName the first name
	...
	public Student fetchByFirstName(String firstName) {
		return fetchByFirstName(firstName, true);
	}

	/**
	 * Returns the student where firstName = &#63; or returns <code>null</code> if it could not be found, optionally using the finder cache.
	 *
	 * @param firstName the first name
	 * @param useFinderCache whether to use the finder cache
	 * @return the matching student, or <code>null</code> if a matching student could not be found
	 */
	@Override
	public Student fetchByFirstName(String firstName, boolean useFinderCache) {
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByFirstName, finderArgs, this);
		}

		if (result instanceof Student) {
			Student student = (Student)result;

			if (!Objects.equals(firstName, student.getFirstName())) {
				result = null;
			}
		}
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						finderCache.putResult(
							_finderPathFetchByFirstName, finderArgs, list);
					}
				}
				else {
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"StudentPersistenceImpl.fetchByFirstName(String, boolean) with parameters (" +
	...
	/**
	 * Removes the student where firstName = &#63; from the database.
	 *
	 * @param firstName the first name
	...
	public Student removeByFirstName(String firstName)
		...
		Student student = findByFirstName(firstName);
	...
	public void cacheResult(Student student) {
		...
		finderCache.putResult(
			_finderPathFetchByFirstName, new Object[] {student.getFirstName()},
			student);
	}
	...
	protected void cacheUniqueFindersCache(StudentModelImpl studentModelImpl) {
		Object[] args = new Object[] {studentModelImpl.getFirstName()};

		finderCache.putResult(
			_finderPathCountByFirstName, args, Long.valueOf(1));
		finderCache.putResult(
			_finderPathFetchByFirstName, args, studentModelImpl);
	}
	...
	public void activate() {
		...
		_finderPathFetchByFirstName = new FinderPath(
			FINDER_CLASS_NAME_ENTITY, "fetchByFirstName",
